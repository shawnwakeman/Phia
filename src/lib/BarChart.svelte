<script lang="ts">
	...
	onMount(async () => {
		...
		function updateDimensions(value, viewBoxResize = false) {
			...
			if (value > 15) {
				...
				if (parentDiv && svgElement) {
					const widthSvg = parentDiv.clientWidth;
					const heightSvg = parentDiv.clientHeight;
					...
					if (!isFirstLoad) {
						...
						d3.select("#circle-packing").attr("width", width).attr("height", height);
		...
		function centerOnNode(node) {
			...
			g.call(zoom.transform, transform).attr("transform", `translate(${x},${y}) scale(${k})`);
		}
		...
		function updateViewBox(newWidth: number, newHeight: number) {
			d3.select("#circle-packing svg").attr("viewBox", `0 0 ${newWidth} ${newHeight}`);
		}
		...
		function updateVisuals() {
			...
			const root = d3
				.hierarchy<WritableNode>(data)
				.sum((d) => d.value ?? 0) // Only use the node's own value
				.sort(function (a, b) {
					return b.data.value - a.data.value;
				});

			const pack = d3.pack<WritableNode>().size([width, height]);
		...
		function shrinkChildrenAndRepack(node) {
			...
			const childrenRoot = d3.hierarchy({ children: node.children }).sum((d) => d.value ?? 0);
		...
		function getAverageColor(children) {
			...
			if (totalPriority === 0) {
				return "#cccccc";
			} // Return a default neutral color
		...
		function getMatchingTargetState(nodeId: number):
			...
			| undefined {
			return $targetStatesStore.find(
				(state) =>
					state.node_id === nodeId && state.snapshot_id === $currentBlock?.snapshot_id
			);
		}
		...
		function updateCircles(nodes: d3.HierarchyCircularNode<WritableNode>) {
			...
			nodes.descendants().forEach((node) => {
				...
				const totalPriority = getNestedIssuesTotalPriority(
					id,
					$nodesDataStore,
					$issuesDataStore
				);
			...
			const gradients = defs
				...
				.data(
					nodes.descendants().filter((d) => d.data.totalPriority !== 0),
					(d) => d.data.id
				)
				.join(
					(enter) =>
						enter.append("radialGradient").attr("id", (d) => `gradient-${d.data.id}`),
					(update) => update,
					(exit) => exit.remove()
				);
			...
			g.selectAll("circle")
				.data(
					nodes.descendants(),
					(d) => (d as d3.HierarchyCircularNode<WritableNode>).data.id
				)
				.join(
				...
				.on("click", (event, d) => {
					if (d.children != null || d.depth == 1) {
						...
						!d.children &&
						d.depth === selectedNode?.depth &&
						d.parent?.data.id === selectedNode.parent?.data.id
					) {
		...
		function convertToNodeType(
			d3Node: d3.HierarchyCircularNode<WritableNode> | null
		): Node | null {
		...
		function updateText(nodes: d3.HierarchyCircularNode<WritableNode>) {
			...
			const isAtSameDepthAndParent = (d) =>
				d.depth === selectedNodeDepth &&
				d.parent?.data.id === selectedNodeParentId &&
				d.data.id !== selectedNodeId;
			const isChildOfSelectedNode = (d) => d.parent?.data.id === selectedNodeId;
			const isSelectedLeafNode = (d) =>
				...
				.join(
					(enter) => {
						const enterGroups = enter
							...
							.attr(
								"transform",
								(d) => `translate(${d.x},${d.y}) scale(${d.r * 0.02})`
							);

						const enterTexts = enterGroups
							.append("text")
						...
						enterTexts.filter((d) => d.r > 10).call(wrap, 10);
							...
							enterGroups
								...
								.text((d) => {
									...
									return hasTargetState
										? `${d.data.state} -> ${hasTargetState.target_state}`
										: "";
								});
					...
					(update) => {
						update
							...
							.attr("class", (d) =>
								d.r > 10
									? "text-node font-default font-medium"
									: "text-node font-default font-small"
							)
							.filter((d) => d.r > 10)
							.call(wrap, 10);

						// Update target state text if it exists
						update.select("text + text").text((d) => {
							const hasTargetState = getMatchingTargetState(d.data.id);
							return hasTargetState
								? `${d.data.state} -> ${hasTargetState.target_state}`
								: "";
		...
		svg.on(
			"wheel.zoom dblclick.zoom mousedown.zoom mousemove.zoom mouseup.zoom touchstart.zoom touchmove.zoom touchend.zoom",
			null
		);
	});
</script>
