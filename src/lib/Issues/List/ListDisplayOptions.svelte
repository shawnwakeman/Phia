<script lang="ts">
    ...
    function handleRowByChange(event) {
        filterStoreList.update(filters => ({ ...filters, rowByField: event.target.value }));
    }

    function handleOrderByChange(event) {
        filterStoreList.update(filters => ({ ...filters, orderByField: event.target.value }));
    }

    function handleOrderDirectionChange(event) {
        filterStoreList.update(filters => ({ ...filters, orderDirection: event.target.value }));
    }

    function handleHideEmptyRowsChange(event) {
        filterStoreList.update(filters => ({ ...filters, hideEmptyRows: event.target.checked }));
    }

    function handleHideNullRowsChange(event) {
        filterStoreList.update(filters => ({ ...filters, hideNullRows: event.target.checked }));
    }
  </script>
