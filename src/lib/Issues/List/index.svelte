<script lang="ts">
	...
	function groupAndSortIssues(data, groupBy, orderBy, orderDir) {
		...
		let result = Object.entries(grouped).map(([key, issues]) => ({
			key,
			issues,
		}));
	...
	function getNestedIssues(
		...
		function findChildNodeIds(nodeId: number, nodes: Node[]): number[] {
			...
			for (let childNode of childNodes) {
				childNodeIds = childNodeIds.concat(
					findChildNodeIds(childNode.id, nodes)
				);
			}
			return childNodeIds;
		}
		...
		const filteredIssues = issues.filter(
			(issue) =>
				issue.node_id !== null && nestedNodeIds.includes(issue.node_id)
		);
	...
	function updateBoard() {
		...
		groupedIssues = groupAndSortIssues(
			filteredIssues,
			filtersFormStore.rowByField,
			filtersFormStore.orderByField,
			filtersFormStore.orderDirection
		);
	}
...
<div class="list-container">
	{#each groupedIssues as { key, issues }}
		<Collapsible.Root open="{true}">
			<div
				class="header-containerr font-default flex items-center justify-between space-x-4 px-6 text-slate-300"
			>
				<h1>
