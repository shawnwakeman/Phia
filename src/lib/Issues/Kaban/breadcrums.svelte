<script lang="ts">
    ...
    function getBreadList(startId: number): { node: Node, showName: boolean }[] {
    ...
    <nav class="breadcrumb-list items-center">
      {#each breadcrumbList as { node, showName }, index}
          {#if showName}
                ...
                <div class="breadcrumb-item p-px text-sm rounded-md transition duration-200 text-slate-300">
                    {node.name}
                </div>
