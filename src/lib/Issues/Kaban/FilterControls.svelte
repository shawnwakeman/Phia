<script lang="ts">
	...
	import { filterStoreKanBan } from "$lib/stores";
	import { SlidersHorizontal } from "lucide-svelte";
	import { Button } from "$lib/components/ui/button/index.js";

	function handleRowByChange(event) {
		filterStoreKanBan.update((filters) => ({
			...filters,
	...
	function handleHideEmptyRowsChange(event) {
		filterStoreKanBan.update((filters) => ({
			...filters,
			hideEmptyRows: event.target.checked,
		}));
	}
	...
	function handleHideEmptyColumnsChange(event) {
		filterStoreKanBan.update((filters) => ({
			...filters,
			hideEmptyColumns: event.target.checked,
		}));
	}
	...
	function handleHideNullRowsChange(event) {
		filterStoreKanBan.update((filters) => ({
			...filters,
			hideNullRows: event.target.checked,
		}));
	}
	...
	function handleHideNullColumnsChange(event) {
		filterStoreKanBan.update((filters) => ({
			...filters,
			hideNullColumns: event.target.checked,
		}));
	}
</script>
...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content>
		<div>
			...
			<select
				id="order-direction"
				on:change="{handleOrderDirectionChange}"
			>
			...
			<div>
				<label>
					<input
						type="checkbox"
						on:change="{handleHideEmptyRowsChange}"
					/>
					Hide Empty Rows
				</label>
			</div>
			...
			<div>
				<label>
					<input
						type="checkbox"
						on:change="{handleHideEmptyColumnsChange}"
					/>
					Hide Empty Columns
				</label>
			</div>
			...
			<div>
				<label>
					<input
						type="checkbox"
						on:change="{handleHideNullRowsChange}"
					/>
					Hide Null Rows
				</label>
			</div>
			...
			<div>
				<label>
					<input
						type="checkbox"
						on:change="{handleHideNullColumnsChange}"
					/>
					Hide Null Columns
				</label>
			</div>
