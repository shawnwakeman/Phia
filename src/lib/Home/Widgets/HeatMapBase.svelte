<script>
		...
		datasets: [
			{
				...
				width: ({ chart }) =>
					((chart.chartArea || {}).right - (chart.chartArea || {}).left) / widthDivisor -
					1,
				height: ({ chart }) =>
					((chart.chartArea || {}).bottom - (chart.chartArea || {}).top) / heightDivisor -
					1,
			},
		],
	};
	...
	onMount(() => {
		...
		const updateCanvasSize = () => {
			showingViz = container.clientWidth >= 200;
