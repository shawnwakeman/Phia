        ...
        embed: {
            ...
            config: {
                services: {
                    ...
                    spotify: {
                    // Regex for Spotify URL (simplified for demonstration; adjust as necessary)
                    regex: /https?:\/\/open\.spotify\.com\/(track|playlist|album|artist)\/([a-zA-Z0-9]+)/,
                    embedUrl: 'https://open.spotify.com/embed/<%= type %>/<%= id %>?utm_source=generator',
                    html: "<iframe style='width: <%= width %>px; height: <%= height %>px;' src='<%= embedUrl %>' frameborder='0' allowtransparency='true' allow='encrypted-media'></iframe>",

                    height: 380,
                    width: 300,
                    // Function to construct the embed URL using detected groups from the regex
                    id: (groups) => {
                        let type = groups[0]; // track, playlist, album, artist
                        let id = groups[1]; // Spotify ID
                        return { type, id };
                    }
                    }
