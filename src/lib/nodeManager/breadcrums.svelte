...
<nav class="flex flex-wrap items-center space-x-2 p-2">
	{#each breadcrumbList as node, index}
		<button
			on:click="{index !== breadcrumbList.length - 1
				? () => navigateTo(node.id)
				: null}"
			class="bg-gray-700 cursor-pointer p-1 text-sm rounded-md transition duration-200 hover:bg-blue-500 hover:text-white {node.id ===
			$selectedNodeId
				? 'text-blue-500'
				: 'text-slate-300'}"
		>
